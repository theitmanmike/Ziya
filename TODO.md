...
- [x] Kimlik doğrulama (Supabase Auth — e-posta + OAuth) — e-posta/şifre tamamlandı. **Google ile giriş (2026-07-29):** kod tarafı tamamen bitti ve doğrulandı — `/auth/callback` route handler (`exchangeCodeForSession`), `AuthForm`'da "Google ile devam et" butonu (PKCE akışı), `handle_new_user()` trigger'ı artık Google'ın `raw_user_meta_data`'sından `full_name`/`name` okuyor (migration `0010_oauth_full_name.sql`). Kullanıcı Google Cloud OAuth istemcisini oluşturup Supabase'de sağlayıcıyı etkinleştirdikten sonra **ikinci bir canlı testte doğrulandı:** buton artık gerçek Google giriş sayfasına yönleniyor ve sayfa doğru şekilde "piuhezrgmoheoaosgmot.supabase.co uygulamasına devam edin" gösteriyor — önceki `"provider is not enabled"` hatası kayboldu, bu da Client ID/Secret eşleşmesinin doğru kurulduğunu kanıtlıyor. **Kalan tek adım (⛔ gerçek bir Google hesabıyla giriş yapmayı gerektirir, ajan bunu yapamaz — kimlik bilgisi girmek yasak):** kullanıcının kendi Google hesabıyla gerçekten "Devam et"e tıklayıp `/dashboard`'a düştüğünü ve `profiles` tablosunda doğru `full_name` ile bir satır oluştuğunu doğrulaması.

**Bilinen ve kabul edilen kozmetik kısıt:** Google'ın hesap seçme ekranındaki "... uygulamasına devam edin" satırı, OAuth consent screen'deki "App name" alanından değil, **redirect URI'nin domain'inden** (`piuhezrgmoheoaosgmot.supabase.co`) okunuyor — bu, Google'ın kullanıcıyı hangi domain'e yönlendirildiği konusunda uyarmak için tasarladığı, tüm üçüncü taraf auth sağlayıcılarını (Supabase dahil) etkileyen standart bir güvenlik davranışı; "App name" alanı doğru ayarlansa bile bu ilk ekranda görünmüyor (yalnızca hesap seçildikten sonraki asıl izin ekranında görünür). Tam düzeltmesi Supabase'in **Custom Domain** özelliğini (Pro plan + ücretli add-on) kullanıp Auth'u `auth.cicibyte.com` gibi kendi domain'imizde barındırmaktan geçiyor. Kullanıcıya maliyet/karar sunuldu, **şimdilik ertelendi** (2026-07-29) — yalnızca kozmetik, girişi engellemiyor.

**Gerçek bug bulundu ve düzeltildi (2026-07-29):** Kullanıcı canlıda Google ile giriş denediğinde Google hesabını seçtikten sonra `ERR_CONNECTION_REFUSED` (localhost bağlantı reddi) hatası aldı. Kök neden Supabase Management API (`GET /v1/projects/piuhezrgmoheoaosgmot/config/auth`) ile doğrulandı: proje `site_url`'i hâlâ `http://localhost:3000` idi ve `uri_allow_list` tamamen **boştu** — yani `https://ziya.cicibyte.com/auth/callback` izin listesinde olmadığından Supabase, `redirect_to` parametresini yok sayıp `site_url`'e (yerel geliştirme adresi) düşüyordu. Kullanıcının onayıyla Management API üzerinden düzeltildi: `site_url` → `https://ziya.cicibyte.com`, `uri_allow_list` → `http://localhost:3000/auth/callback,https://ziya.cicibyte.com/auth/callback`. **Canlıda doğrulandı:** `ziya.cicibyte.com/login`'den Google butonuna tıklandığında Google'a giden `redirect_to` parametresi artık doğru şekilde `https://ziya.cicibyte.com/auth/callback?next=/dashboard` — localhost hiç geçmiyor.
